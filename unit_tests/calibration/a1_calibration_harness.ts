...
type CalibrationSummary = {
  evaluatedConfig: A1Config;
  scenarios: {
    stepResponse: unknown;
    outlierDamping: unknown;
    trustDrift: unknown;
    decayTimeline: unknown;
    groupRecency: unknown;
  };
};
...
async function runStepResponseScenario(config: A1Config) {
  const start = new Date("2026-03-28T14:00:00.000Z");
  const harness = createHarness(config);
  const users = ["user-a", "user-b", "user-c"];
  const series: Array<{ minute: number; aggregateNoise: number | null }> = [];

  for (let minute = 0; minute <= 12; minute += 1) {
    const avgNoise = minute < 5 ? 45 : 65;
    for (const [index, userId] of users.entries()) {
      harness.reportRepository.insert(
        makeReport(`step-${minute}-${index}`, userId, "loc-a", {
    ...
    const location = result.updatedStudyLocations.find(
      (candidate) => candidate.studyLocationId === "loc-a",
    );
...
async function runOutlierDampingScenario(config: A1Config) {
  ...
  const storedOutlier = harness.reportRepository.snapshot().find(
    (record) => record.report.reportId === outlierReport.reportId,
  );
  ...
  const dampingRatio =
    aggregateNoise === null ? null : (aggregateNoise - baselineMean) / (outlierReport.avgNoise - baselineMean);
...
async function runTrustDriftScenario(config: A1Config) {
  ...
  const series: Array<{
    cycle: number;
    stableNoiseWF: number;
    overReporterOccupancyWF: number;
    underReporterNoiseWF: number;
  }> = [];

  for (let cycle = 0; cycle < 8; cycle += 1) {
    const timestamp = minutesAfter(start, cycle);
    harness.reportRepository.insert(
      makeReport(`stable-${cycle}`, "stable-user", "loc-a", {
        createdAt: timestamp,
        avgNoise: 55,
        maxNoise: 60,
        variance: 2,
    ...
    harness.reportRepository.insert(
      makeReport(`over-${cycle}`, "over-reporter", "loc-a", {
        createdAt: timestamp,
        avgNoise: 55,
        maxNoise: 60,
        variance: 2,
    ...
    harness.reportRepository.insert(
      makeReport(`under-${cycle}`, "under-reporter", "loc-a", {
        createdAt: timestamp,
        avgNoise: 47,
        maxNoise: 51,
        variance: 2,
...
async function runDecayTimelineScenario(config: A1Config) {
  ...
  harness.reportRepository.insert(
    makeReport("decay-a", "user-a", "loc-a", {
      createdAt: start,
      avgNoise: 52,
      occupancy: 3,
    }),
  ...
  harness.reportRepository.insert(
    makeReport("decay-b", "user-b", "loc-b", {
      createdAt: start,
      avgNoise: 58,
      occupancy: 4,
    }),
  ...
  const series: Array<{
    minute: number;
    locationANoise: number | null;
    locationBNoise: number | null;
    groupNoise: number | null;
  ...
  const expectedDropMinute = Math.ceil(
    (config.reportHalfLifeMs * Math.log(config.initialDecayWF / config.minWeightThreshold)) /
      (Math.log(2) * 60_000),
  );
...
async function runGroupRecencyScenario(config: A1Config) {
  const now = new Date();
  const weights = [0, 30, 60, 120, 180, 240];
  const series: Array<{
    staleAgeSeconds: number;
    aggregateNoise: number | null;
    aggregateOccupancy: number | null;
  }> = [];
...
function loadConfigOverride(): A1Config {
  const overrideArg = process.argv[2];
  if (!overrideArg) {
    return { ...defaultA1Config };
  }

  const overridePath = path.resolve(process.cwd(), overrideArg);
  const parsed = JSON.parse(fs.readFileSync(overridePath, "utf8")) as Partial<A1Config>;
  return {
    ...defaultA1Config,
    ...parsed,
    componentWeights: {
      ...defaultA1Config.componentWeights,
...
function firstMinuteAtOrAbove(
  series: Array<{ minute: number; aggregateNoise: number | null }>,
  threshold: number,
): number | null {
  const match = series.find(
    (entry) => entry.aggregateNoise !== null && entry.aggregateNoise >= threshold,
  );
  return match ? match.minute : null;
}

function firstMinuteWithNull(series: Array<{ minute: number; value: number | null }>): number | null {
...
function indexByUserId(users: User[]): Map<string, User> {
  return new Map(users.map((user) => [user.userId, user]));
}

function classifyDampingRatio(dampingRatio: number): string {
  if (dampingRatio <= 0.15) return "strong damping";
  if (dampingRatio <= 0.35) return "moderate damping";
  return "weak damping";
}

function assessStepResponse(
  series: Array<{ minute: number; aggregateNoise: number | null }>,
  baseline: number,
  target: number,
): string {
...
function assessTrustDrift(
  final: { overReporterOccupancyWF: number; underReporterNoiseWF: number },
  series: Array<{ overReporterOccupancyWF: number; underReporterNoiseWF: number }>,
): string {
  const occupancyShift = Math.abs(final.overReporterOccupancyWF - 1);
  const noiseShift = Math.abs(final.underReporterNoiseWF - 1);
  const perCycleSpike = Math.max(
    maxStepDelta(series.map((entry) => entry.overReporterOccupancyWF)),
    maxStepDelta(series.map((entry) => entry.underReporterNoiseWF)),
  );
...
function assessDecayPace(
  series: Array<{ minute: number; locationANoise: number | null; groupNoise: number | null }>,
  expectedDropMinute: number,
): string {
  const actual = firstMinuteWithNull(
    series.map((entry) => ({ minute: entry.minute, value: entry.locationANoise })),
  );
...
function assessGroupRecency(
  series: Array<{ staleAgeSeconds: number; aggregateNoise: number | null }>,
): string {
