...
void main() {
  group('data collection workflow', () {
    test('session service initializes a local session from coordinates', () async {
      ...
      expect(service.activeStudyLocation?.studyLocationId, 'library-floor-1-quiet');
      expect(service.currentUser?.displayName, 'Local User');
      expect(service.sessionState, isNotNull);
    });

    test('session service builds a report from the active session window', () async {
      final service = SessionService();
    ...
    test('session state tracks local capture fields and updates immutably', () {
      ...
      expect(updatedSession.studyLocationId, seededStudyLocations.first.studyLocationId);
