...
export interface LocationResolutionConfig {
  maxResolutionDistanceMeters: number;
}

export interface LocationResolutionResult {
  resolvedStudyLocation: StudyLocation | null;
  distanceMeters: number | null;
}

export class LocationResolutionService {
  constructor(private readonly config: LocationResolutionConfig) {}

  resolveNearestStudyLocation(
    ...
    let nearestLocation: StudyLocation | null = null;
  ...
  resolveNearestStudyLocationWithinGroup(
    userCoordinates: Coordinates,
    studyLocations: StudyLocation[],
    locationGroupId: string,
  ): LocationResolutionResult {
    const groupLocations = studyLocations.filter(
...
export interface NoiseSummaryConfig {
...
export class NoiseSummaryService {
  constructor(private readonly config: NoiseSummaryConfig) {}

  summarize(rawSamples: number[]): NoiseSummary {
...
export interface SessionCorrectionServiceConfig {
  peerWindowMs: number;
...
export interface SessionCorrectionNoiseWFContext {
  report: Report;
...
export class SessionCorrectionService {
  constructor(private readonly config: SessionCorrectionServiceConfig) {}

  computeSessionCorrectionNoiseWF(
    context: SessionCorrectionNoiseWFContext,
  ): number {
  ...
  evaluate(
    ...
    const historicalBaselineNoise =
      historicalPeers.length > 0 ? mean(historicalPeers.map((peer) => peer.avgNoise)) : null;
    const peerBaselineNoise =
      currentPeers.length > 0 ? mean(currentPeers.map((peer) => peer.avgNoise)) : null;

    const historicalScore =
      historicalBaselineNoise === null
        ? 1.0
        : this.scoreDeviation(
            Math.abs(context.report.avgNoise - historicalBaselineNoise),
            this.config.historicalToleranceDb,
          );

    const peerScore =
      currentPeers.length < this.config.minPeerCountForPeerScore || peerBaselineNoise === null
        ? 1.0
        : this.scoreDeviation(
            Math.abs(context.report.avgNoise - peerBaselineNoise),
            this.config.peerToleranceDb,
          );

    const userScore = this.computeUserScore(context.user);

    const weightedScore = weightedAverage(
      [historicalScore, userScore, peerScore],
      [
    ...
    return {
      sessionCorrectionNoiseWF: clamp(
        weightedScore,
        this.config.minSessionCorrectionWF,
        1.0,
      ),
      historicalScore,
      userScore,
  ...
  private selectCurrentPeers(report: Report, reportHistory: Report[]): Report[] {
    return reportHistory.filter((candidate) => {
      if (candidate.reportId === report.reportId) return false;
      if (candidate.studyLocationId !== report.studyLocationId) return false;

      const ageDifferenceMs = Math.abs(
        candidate.createdAt.getTime() - report.createdAt.getTime(),
      );
  ...
  private selectHistoricalPeers(report: Report, reportHistory: Report[]): Report[] {
    ...
    return reportHistory.filter((candidate) => {
      ...
      if (candidateAgeMs <= this.config.peerWindowMs) return false;
      if (candidateAgeMs > historicalLookbackMs) return false;

      const candidateMinuteOfWeek = minuteOfWeek(candidate.createdAt);
      const minuteDifference = circularMinuteDistance(
        reportMinuteOfWeek,
        candidateMinuteOfWeek,
        7 * 24 * 60,
      );
  ...
  private scoreDeviation(deviationDb: number, toleranceDb: number): number {
    if (toleranceDb <= 0) {
  ...
  private computeUserScore(user: User): number {
    const deviationFromNeutral = Math.abs(
      user.userNoiseWF - this.config.userNoiseWFNeutral,
    );
...
export const defaultNoiseSummaryConfig: NoiseSummaryConfig = {
...
export const defaultSessionCorrectionServiceConfig: SessionCorrectionServiceConfig = {
...
function sanitizeNoiseSamples(rawSamples: number[]): number[] {
